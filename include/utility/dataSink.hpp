...
namespace dontTouchThisPleaseIgnoreIt {
    ...
    class DataSink
    {
        ...
        void parseData() {
            ...
            // Setup file prerequistes:
            std::string fileName = this->id + ".csv";
            std::string dataBuffer = this->headerText;
            std::vector<std::string> secondaryBuffer;

            // add to data buffer columns
            for (size_t i = 1; i < this->session.size(); ++i) {
                dataBuffer += ", data" + to_string(static_cast<int>(i));
            }
            dataBuffer += "\n"; // end columns

            dataStructure firstStructure = this->session.at(0);
            ...
            for (const auto& dataPoint : firstStructure.data) {
                ...
                tempBuffer += "\n";
;
                secondaryBuffer.push_back(tempBuffer);
            ...
            for (const auto& dataPoint : firstStructure.data) {
                ...
                dataBuffer += secondaryBuffer.at(index);
...
template<typename K, typename T>
class DataSink_Motor : public Bucees::dontTouchThisPleaseIgnoreIt::DataSink<K, T> {
    private:
    ...
    public:
    DataSink_Motor(std::string id, std::vector<std::map<K, T>> dataCaptures, int timeStep = 500) : Bucees::dontTouchThisPleaseIgnoreIt::DataSink<K, T>(id, dataCaptures, timeStep) {}
};

template<typename K, typename T>
class DataSink_Robot : public Bucees::dontTouchThisPleaseIgnoreIt::DataSink<K, T> {
    private:
    double getData(K target, T key) override { // alpha sigma function >:C
    ...
    public:
    DataSink_Robot(std::string id, std::vector<std::map<K, T>> dataCaptures, int timeStep = 500) : Bucees::dontTouchThisPleaseIgnoreIt::DataSink<K, T>(id, dataCaptures, timeStep) {}
};