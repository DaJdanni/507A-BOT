	...
	if(fp != NULL) {
		...
		if(_gifmem != NULL) {
			...
			if(_buffer == NULL) {
				...
				_t1 = vex::thread(_render_task, static_cast<void*>(this));
			}
		}
	} else {
...
void Gif::_render() {
	...
	for (size_t looped = 1;; looped++) {
		while (gd_get_frame(_gif)) {
			...
			int32_t delta = vex::timer().system() - now;
			delay -= delta;

			if(delay > 0) vex::this_thread::sleep_for(delay);
		}
